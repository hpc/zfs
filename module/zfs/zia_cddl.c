    ...
    uint8_t level, boolean_t *local_offload)
{
	...
	if (!ABD_HANDLE(src)) {
		...
		const int ret = zia_offload_abd(props->provider, src, s_len,
		...
		int ret = dpusm->all_zeros(ABD_HANDLE(src), 0, s_len);
		if (ret == DPUSM_OK) {
			*c_len = 0;
			return (ZIA_OK);
	...
	*cbuf_handle = zia_alloc(props->provider,
	...
	const int ret = dpusm->compress(compress_to_dpusm(c), (int8_t)level,
